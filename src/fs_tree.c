...
FS_STATUS fs_tree_file_create(fs_handle_t *fs, fs_general_file_handle_t *parent, const char *name, fs_general_file_handle_t *result)
{
    ...
    TRUE_THEN_RETURN_FALSE(fs_general_file_write(fs->block, fs->superblock, parent, &(parent->block_first), 4) != 4);            // 写入父目录地址
    return true;
}
...
{
    uint32_t next_block;
    TRUE_THEN_RETURN_FALSE(tree == NULL);
...
{
    TRUE_THEN_RETURN_FALSE(fs == NULL);
    TRUE_THEN_RETURN_FALSE(tree == NULL);
    TRUE_THEN_RETURN_FALSE(tree->header.magic != FS_BLOCK_TREE_MAGIC);
...
FS_STATUS fs_tree_rmdir(fs_handle_t *fs, fs_general_file_handle_t *tree)
{
    ...
    TRUE_THEN_RETURN_FALSE(fs_general_file_remove(fs->block, fs->superblock, tree->block_first));
    return true;
}
